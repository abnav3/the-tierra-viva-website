...
const MAKE_WEBHOOK_URL = 'https://hook.eu1.make.com/u7tyy52159tb3u9uzf65kuwt1ysc439v';
...
function sendBooking(e){
  ...
  const m=parseInt(room.match(/\$(\d+)/)?.[1]||'0',10);
  const total=(m+Math.max(0,guests-2)*300)*nights;
  ...
  fetch(MAKE_WEBHOOK_URL,{
    ...
    body:JSON.stringify({room,guests,name,phone,pay,checkin,checkout,nights,total,lang:currentLang})
  }).then(r=>r.text()).then(()=>{
    btn.textContent='\u2705 Solicitado \u2014 Te responderemos al WhatsApp';
